...
uint8_t *fido_pool_buffer = NULL;

#ifdef SD_SAVE
void load_fido_from_sd(void)
{
    return load_flash_from_sd();
    show_message("loading files      ");
    FILE *f = fopen("/sdcard/fido.db", "rb");
    if (f)
    {
        fseek(f, 0, SEEK_END);
        long size = ftell(f);
        ...
        file_t *fido_ef = search_file(EF_PHY);
        if (fido_ef)
        {
            flash_write_data_to_file(fido_ef, buffer, (uint16_t)size);
        }

        free(buffer);
    }
    show_message("loaded files     ");
}
#endif // SD_SAVE
...
// TODO gavhu10 this is interesting. maybe disable pin code?
