...
static uintptr_t allocate_free_addr(uint16_t size, bool persistent)
{
    ...
    for (uintptr_t base = endp; base >= startp; base = next_base)
    {
        ...
        { // we are at the end
            ...
            { // check whether it fits in the next sector, so we take addr_aligned as the base
...
{
#ifdef SD_SAVE
    if (file_get_size(file) == 256) // just make sure its a key
    {
        show_message("saving file    ");
        FILE *f = fopen("/sdcard/fido.db", "wb");
        if (f != NULL)
        {
            fwrite(data, 1, len, f);
            fclose(f);
        ...
        {
            show_message("error opening db    ");
            delay(1000);
        }
        show_message("done saving file   ");
        delay(1000);
    }
#endif //  SD_SAVE
